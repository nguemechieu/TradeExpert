...
extern bool    High_Impack    = true;                 // Include high
extern bool    Medium_Impack     = true;                 // Include medium
extern bool    Low_Impack      = true;                 // Include low
extern bool    Include_Speaks     = true;                 // Include speaks
extern bool    Include_Holidays   = false;                // Include holidays
...
extern bool    AllowSubwindow    = true;                // Show Panel in sub window
extern ENUM_BASE_CORNER Corner   = 2;                    // Panel side
extern string  PanelTitle="Market News Event"; // Panel title
...
extern color   NegativeColor     = clrRed;            // Negative forecast color
   ...
   while(true)
     {
      ...
      if(!IsCurrency(Event[index][COUNTRY]))
         continue;
      if(!High_Impack && Event[index][IMPACT]=="High")
         continue;
      if(!Medium_Impack&& Event[index][IMPACT]=="Medium")
         continue;
      if(!Low_Impack && Event[index][IMPACT]=="Low")
         continue;
      if(!Include_Speaks && StringFind(Event[index][TITLE],"Speaks")!=-1)
         continue;
      if(!Include_Holidays && Event[index][IMPACT]=="Holiday")
         continue;
