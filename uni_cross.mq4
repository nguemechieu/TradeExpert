...
input bool UseSound              = true;           // Alerts plays a sound ? 
input bool TypeChart             = true;           // Alerts displays on screen ?
input bool UseAlert              = true;           // Use alert?
...
//---- indicators
  ...
  int index = 0;
   ...
   status = T3.OnInit(index,T3Period,T3Price,bFactor);
   if (status != INIT_SUCCEEDED) {
      return status;
   }   
